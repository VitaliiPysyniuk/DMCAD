...
def read_file(filename):
    ...
    for i in range(len(weights_list)):
        for j in range(len(weights_list)):
            if i == j:
                weights_list[i][j] = np.Infinity
            if weights_list[i][j] == 0:
                weights_list[i][j] = 999

    return np.array(weights_list, float)


def print_result(tour, weight_matrix):
    route_points = list()
    previous_place = 0
    i = 0
    while tour:
        if tour[i]['from'] == previous_place:
            route_points.append(tour[i]['from'])
    ...
    length = 0
    print(f'Direction: {string.ascii_uppercase[route_points[0]]}', end='')
    for i in range(1, len(route_points)):
        print(f' -> {string.ascii_uppercase[route_points[i]]}', end='')
        length += weight_matrix[route_points[i - 1]][route_points[i]]
    print(f'\n   Length: {int(length)}', end='')
...
if __name__ == '__main__':
    ...
    print_result(tour, weight_matrix)
